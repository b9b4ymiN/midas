...
name: har-to-api
description: >
  The data layer — pull a company's facts reproducibly, with every figure carrying its source,
  as-of date and URL, and reverse-engineer a site's hidden JSON API when a new source is needed.
  Two jobs, one toolkit. Use for the RUN-TIME job whenever data is needed for analysis: "pull the
  data for TU", "fetch financials for a ticker", "get the numbers before we value this", "why do
  my numbers change between runs", "where did this figure come from", "re-run that analysis on the
  same data", or as Step 0 of both-stock-analysis. Use for the SETUP job when adding a source:
  "this site has no API, how do I call it", "build a client from a HAR file", "automate a
  financial dashboard" (stockanalysis.com, valueinvesting.io, gurufocus.com, finviz.com), or on
  seeing a screenshot about deriving API clients from HAR capture. Ships six verified
  stockanalysis endpoints covering US and Thai listings — no capture needed to start.
---
