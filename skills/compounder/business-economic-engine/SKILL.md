...
7d. **Life-cycle stage — mandatory reading.** Classify the stage from the signs of operating, investing and financing cash flow per `references/life-cycle-stage.md`, on investing cash flow **excluding** securities purchases and sales, and report the unadjusted classification beside it. Where the two differ, say so — the difference means treasury activity is being folded into the same line as capital spending. In Introduction and Decline the reinvestment rate is divided by a NOPAT that carries no information, so those stages force the unit-economics path. The stage is a prior for Duration and a gate on measure validity; it never selects the business frame and never sets the Potential label.
